...
res_cols <- c("NN", "time_estimation", "sigma2", "rho", "time_negLL", "negLL_at_true_covpars", "num_optim_iter", 
              "time_prediction", "RMSE_latent_mu", "log_score", "number_of_errors", "number_of_neg_pred_var")
...
for(r in 1:n_rep){
  ...
  for(j in 1:length(NN)){
    ...
    results_iterative_VL$time_estimation[k] <- system.time(model_iterative_VL$fit(y=y_train))[3]
    ...
    results_iterative_VL$time_prediction[k] <- system.time(
    ...
    results_GPVecchia$time_estimation[k] <- system.time({
    ...
    ##Prediction
    results_GPVecchia$time_prediction[k] <- system.time({
