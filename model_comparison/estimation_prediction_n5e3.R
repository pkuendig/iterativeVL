...
# Compare models: Laplace | Cholesky-VL | Iterative-VL | GPVeccia (V0.1.4)
################################################################################
...
res_cols <- c("time_estimation", "sigma2", "rho", "negLL_at_true_covpars", "num_optim_iter", 
              "time_prediction", "RMSE_latent_mu", "log_score", "number_of_errors")
...
for(r in 1:n_rep){
  ...
  results_Laplace$time_estimation[i] <- system.time(model_Laplace$fit(y=y_train))[3]
  ...
  ##Prediction
  results_Laplace$time_prediction[i] <- system.time(
  ...
  results_VL$time_prediction[i] <- system.time(
  ...
  results_iterative_VL$time_prediction[i] <- system.time(
  ...
  ##Prediction
  results_GPVecchia$time_prediction[i] <- system.time({
  ...
  results_GPVecchia$log_score[i] <- -sum(dnorm(b_test, preds_test$mu.pred, sqrt(preds_test$var.pred), log = TRUE)) #negative variances!
